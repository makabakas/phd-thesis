...
for jj=1:length(X)
    p = zeros(size(t));
    for ii=1:length(t)
        p(ii) = sound_field_imp(X(jj),Y(jj),Z(jj),x0,'ps',d,t(ii),conf);
        progress_bar(ii+(jj-1)*length(t),5*runs);
    end
    gp_save(sprintf('data/nfchoa_X%.2f_Y%.2f.txt',X(jj),Y(jj)),[t p]);
end
% --- WFS ---
...
for jj=1:length(X)
    ...
    for ii=1:length(t)
        ...
        progress_bar(ii+(jj-1)*length(t)+runs,5*runs);
    end
    gp_save(sprintf('data/wfs_X%.2f_Y%.2f.txt',X(jj),Y(jj)),[t p]);
end

%% ===== Sampled secondary sources =======================================
...
for jj=1:length(X)
    ...
    for ii=1:length(t)
        p(ii) = sound_field_imp(X(jj),Y(jj),Z(jj),x0,'ps',d,t(ii),conf);
        progress_bar(ii+(jj-1)*length(t)+2*runs,5*runs);
    end
    gp_save(sprintf('data/nfchoa_X%.2f_Y%.2f_nls64.txt',X(jj),Y(jj)),[t p]);
end
...
for jj=1:length(X)
    ...
    for ii=1:length(t)
        p(ii) = sound_field_imp(X(jj),Y(jj),Z(jj),x0,'ps',d,t(ii),conf);
        progress_bar(ii+(jj-1)*length(t)+3*runs,5*runs);
    end
    gp_save(sprintf('data/nfchoa_X%.2f_Y%.2f_nls64_band_limited.txt',X(jj),Y(jj)),[t p]);
end
% --- WFS ---
x0 = secondary_source_selection(x0,xs,src);
x0 = secondary_source_tapering(x0,conf);
...
for jj=1:length(X)
    ...
    for ii=1:length(t)
        ...
        % filter
        p(ii) = sound_field_imp(X(jj),Y(jj),Z(jj),x0,'ps',d,t(ii)+64/conf.fs,conf);
        progress_bar(ii+(jj-1)*length(t)+4*runs,5*runs);
    end
